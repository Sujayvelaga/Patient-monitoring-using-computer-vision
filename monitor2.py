...
CONFIG = {
    ...
    # Email alerts
    "SEND_EMAIL":           True,
    "email_sender":         "sujayvelaga@gmail.com",
    "email_password":       "czol ipwi omeb umuz",
    "email_receiver":       "goldfish81106@gmail.com",
...
class SalineDetector:
    ...
    def detect(self, frame):
        ...
        score = cv2.compareHist(self.calib, hist, cv2.HISTCMP_BHATTACHARYYA)
        self.change_pct = min(score*100, 100)
        self.history.append(self.change_pct)
        avg = float(np.mean(self.history))
        self.level = max(0, int(100 - avg*1.5))
...
class FallDetector:
    ...
    def reset_bg(self):
        ...
        self.posture = "UNKNOWN"
        self._last_box = None
        print("  Background / Pose model reset.")
    ...
    def detect(self, frame, pose_results):
        ...
        x_min = min([l.x for l in lm]) * w
        x_max = max([l.x for l in lm]) * w
        y_min = min([l.y for l in lm]) * h
        y_max = max([l.y for l in lm]) * h
        
        bx, by = int(x_min), int(max(0, y_min))
        ...
        self._last_box = (bx,by,bw,bh,aspect)
        ...
        cx,cy = bx, max(by-10,20)
        (tw,th),_ = cv2.getTextSize(badge,cv2.FONT_HERSHEY_SIMPLEX,0.52,2)
        cv2.rectangle(frame,(cx-2,cy-th-4),(cx+tw+2,cy+2),DARK_BG,-1)
        cv2.putText(frame, badge,(cx,cy),cv2.FONT_HERSHEY_SIMPLEX,0.52,col,2)
...
class NoMovementDetector:
    ...
    def detect(self, frame, pose_results):
        ...
        # Track 4 core joints (Shoulders and Hips) to determine movement 
        ...
        diffs = np.linalg.norm(curr_points - self.prev_points, axis=1)
        self.score = float(diffs.mean())
...
def run(source):
    ...
    while True:
        ...
        frame, fs = fall.detect(frame, pose_results)
        frame, ms = nomove.detect(frame, pose_results)
